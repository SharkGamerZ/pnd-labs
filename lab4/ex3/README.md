An internal lan with some hosts (pc1 192.168.100.10, 2001:db8:cafe:1::10),
a DMZ (203.0.113.0/24, 2001:db8:cafe:2::/64) with two servers,
...
3.Internal users can access the Web servers and mail servers in DMZ via SSH,
  ...
  hosts in DMZ can only be contacted on port 22 by hosts in the internal
  lan.

4.You can test your policy from the isp machine or placing the host machine
  into the 'external' network (using the connect-lab.sh script).
